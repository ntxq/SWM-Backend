...
export class ProgressManager{
	...
	getProgressClass(requestID:number,cutIndex:number){
		if(this.progress_map.has(requestID) === false){
			this.progress_map.set(requestID,new Map<number,Progress>());
		}
		const progress_request = this.progress_map.get(requestID) as Map<number,Progress>
	...
	getProgress(requestID:number,cutIndex:number):number{
		var progress = this.getProgressClass(requestID,cutIndex).getProgress()
		//0~200사이 범위로 제한
		progress = Math.max(0,Math.min(200,progress))
		return progress
	}

	updateProgress(requestID:number,cutIndex:number,status:string){
		this.getProgressClass(requestID,cutIndex).updateStatus(status)
	}

	updatePart(requestID:number,cutIndex:number,status:string){
		if(this.progress_map.get(requestID)?.has(cutIndex)){
			this.getProgressClass(requestID,cutIndex).updatePart(status);
		}
	}

	updateTotalPart(requestID:number,cutIndex:number,status:string,count:number){
		if(this.progress_map.get(requestID)?.has(cutIndex)){
			this.getProgressClass(requestID,cutIndex).updateTotalPart(status,count);
		}
	}
	
	restoreTotalPart(requestID:number,cutIndex:number,status:string){
		if(this.progress_map.get(requestID)?.has(cutIndex)){
