...
export class SegmentationInterface {
  ...
  async Start(req_id: number, callback?: Function | undefined) {
    fs.readFile(
      await queryManager.get_path(req_id, "original"),
      (err, data) => {
        if (err) {
          console.error(err);
          return;
  ...
  async UpdateMask(
    ...
    await queryManager.update_progress(req_id, "cut");
  ...
  UpdateProgress(
    call: grpc.ServerUnaryCall<
      MESSAGE.SendUpdateProgress,
      MESSAGE.ReplySendUpdateProgress
    >,
...
export class OCRInterface {
  ...
  JsonTransfer(
    ...
    switch (request.type) {
      case "bbox":
        queryManager
          .set_bboxes(request.req_id, JSON.parse(request.data))
          .then(() => {
