...
export class mysqlConnectionManager {
  ...
  add_original_sources(project_id: number, files: Express.Multer.File[]) {
    ...
    for (var i = 0; i < files.length; i++) {
      ...
      const procedure: Procedure = {
        ...
        callback: (rows: any, err: any) => {
          ...
          const new_path = `${IMAGE_DIR}/original/${req_id}${path.extname(
            file.originalname
          )}`;
    ...
    return new Promise<[Map<string, number>, Map<number, string>]>(
      (resolve, reject) => {
    ...
    return new Promise<string>((resolve, reject) => {
      const procedure: Procedure = {
        ...
        callback: (rows: any, err: any) => {
          ...
          const complete: string = String(rows["complete"]);
          resolve(complete);
        },
      };
      mysql_connection.callProcedure(procedure);
    ...
    for (const bbox of bboxList) {
      var procedure: Procedure = {
        ...
        parameters: [
          ...
          bbox.translatedX,
          bbox.translatedY,
          bbox.translatedWidth,
          bbox.translatedHeight,
          bbox.translatedText,
          bbox.fontColor,
          bbox.fontSize,
          bbox.fontFamily,
